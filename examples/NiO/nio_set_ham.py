...
from fcdmft.gw.mol import ugw_dc
...
# NOTE: set up KRKS by user
# obtain spin-restricted IAO basis (currently required by gw_dc)
chkfname = 'nio_444_rks.chk'
if os.path.isfile(chkfname):
    krmf = dft.KRKS(cell, kpts).density_fit()
    krmf = scf.addons.smearing_(krmf, sigma=5e-3, method="fermi")
    krmf.xc = 'pbe'
    krmf.with_df = gdf
    krmf.with_df._cderi = gdf_fname
    krmf.conv_tol = 1e-12
    data = chkfile.load(chkfname, 'scf')
    krmf.__dict__.update(data)
else:
    krmf = dft.KRKS(cell, kpts).density_fit()
    krmf = scf.addons.smearing_(krmf, sigma=5e-3, method="fermi")
    krmf.xc = 'pbe'
...
# NOTE: choose IAO basis and use krmf by user
# C_ao_lo: transformation matrix from AO to LO (IAO) basis
MINAO = {'Ni':'gth-szv-molopt-sr', 'O':'gth-szv-molopt-sr'}
C_ao_iao, C_ao_iao_val, C_ao_iao_virt = make_basis.get_C_ao_lo_iao(Lat, krmf, minao=MINAO, full_return=True)
...
kmf_hf = scf.KUHF(cell, kpts, exxdiv=None)
kmf_hf.with_df = gdf
kmf_hf.with_df._cderi = gdf_fname
kmf_hf.max_cycle = 0
JK_ao = np.asarray(kmf_hf.get_veff(dm_kpts=DM_ao))
if len(JK_ao.shape) == 3:
    JK_ao = JK_ao[np.newaxis, ...]
...
if gw_fc:
    ...
    nocca = (cell.nelectron+cell.spin) // 2
    noccb = cell.nelectron - nocca
...
gw.ef = ef
gw.fullsigma = True
omega = np.linspace(6./27.211386,24./27.211386,91)
...
for i in range(len(omega)):
    print (omega[i], np.trace(sigma_lo[0,:,:,i].imag), np.trace(sigma_lo[0,:,:,i].real))
